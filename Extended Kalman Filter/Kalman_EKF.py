...
class System:
    def __init__(self):
        ...
        # self.mag_Ainv = np.array([[ 1.34406, 0.01407, 0.02209],
        #                           [ 0.01407, 1.46499, 0.04752],
        #                           [ 0.02209, 0.04752, 1.47276]])
        # self.mag_b = np.array([-1.58358, 3.3255, 60]).transpose()
        
        # self.mag_Ainv = np.array([[ 1, 0.01407, 0.02209],
        #                           [ 0.01407, 1, 0.04752],
        #                           [ 0.02209, 0.04752, 1]])
        # self.mag_b = np.array([0 , 0, 0]).transpose()

        # self.mag_Ainv = np.array([[ 1.72282984, -0.01124267, 0.01198653],
        #                           [ -0.01124267, 1.6410486, -0.00574405],
        #                           [ 0.01198653, -0.00574405, 1.65152302]])
        # self.mag_Ainv = np.array([[ 0.5, 0, 0],
        #                           [ 0, 0.5, 0],
        #                           [ 0, 0, 0.5]])
        
        # self.mag_b = np.array([-277.8, -250.2, -133.79999999999998]).transpose()

        self.mag_Ainv = np.array([[ 0.47018389 ,-0.00450253 ,-0.0025526],
                                [-0.00450253 , 0.45756392 ,-0.00656173],
                                [-0.0025526  ,-0.00656173 , 0.45312696]])
        self.mag_b = np.array([  -57, 27.04626471  , -87.2181])
    
        '''
        self.mag_Ainv = np.array([[ 0.45833711, -0.00594221, -0.0023387],
                    [  0.00594221, 0.4529516,  -0.00494346],
                    [ -0.0023387, -0.00494346, 0.45233592]])
        self.mag_b = np.array([1.9, 5  , -180])
        '''
    ...
    def getMagVector(self, m):
        # print(int(m[0]),int(m[1]),int(m[2]))
        magGaussRaw = np.matmul(self.mag_Ainv, (np.array(m).transpose() - self.mag_b))
        # print(int(magGaussRaw[0]),int(magGaussRaw[1]),int(magGaussRaw[2]))
